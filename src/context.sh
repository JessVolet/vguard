...
seleccionar_contexto() {
    ...
    local fallback_s_name
    fallback_s_name="$(basename "${ruta_target%/}")"
    local raw_s_name="${VGUARD_SERVICE_NAME:-$fallback_s_name}"
    ...
    if [ -z "$s_name" ] || [ "$s_name" = "." ]; then
        s_name="$fallback_s_name"
    fi
